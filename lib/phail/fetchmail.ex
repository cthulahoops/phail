defmodule Phail.Fetchmail do
  ...
  def gen_config_file(filename) do
    ...
    for account <- MailAccount.all() do
      IO.puts(
        file,
        "poll #{inspect(account.fetch_server)} protocol #{Atom.to_string(account.fetch_protocol)} user #{
          inspect(account.fetch_username)
        } there with password #{inspect(account.fetch_password)} mda \"#{mda} #{
          account.user.email
        }\" fetchall\n"
      )
    end
