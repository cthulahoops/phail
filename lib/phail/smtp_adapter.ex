defmodule Phail.SMTPAdapter do
  @moduledoc """
  ...
  *Sensitive credentials should not be committed to source control and are best kept in environment variables.
  ...
  Adapted from Bamboo.SMTPAdapter which available under The MIT License, and Copyright (c) 2016 Fewlines SAS.
  """
  ...
  defp smtp_server_config(%Bamboo.Email{from: {_name, address}}) do
    config = Phail.MailAccount.get_by_email(address)

    [
      relay: config.smtp_server,
      port: config.smtp_port,
      username: config.smtp_username,
      password: config.smtp_password,
      tls: if(config.smtp_ssl, do: :always, else: :never),
      auth: :always,
